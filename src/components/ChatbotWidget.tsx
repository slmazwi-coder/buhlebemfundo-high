...
// ── Claude AI (Anthropic) ────────────────────────────────────────────────────
const SYSTEM_PROMPT = `You are a warm, knowledgeable and friendly assistant for Buhlebemfundo Secondary School in KwaDabeka, KwaZulu-Natal, South Africa.
...
- Location: 516 Kululeka Drive, KwaDabeka, 3610
- Phone: 031 171 0207
- Email: buhlebemfundosec@gmail.com
- Motto: "Knowledge is Power"
- Principal: Mr. S. Dlamini
- School hours: Monday–Thursday 07:30–15:30, Friday 07:30–13:30
- Grades: Grade 8 to Grade 12
- 2026 applications currently open (general and boarding)
- 2025 Matric pass rate: 93.4% | Bachelor passes: 112 | Distinctions: 215
...
async function askClaude(userMessage: string): Promise<string> {
  try {
    ...
    return 'I\'m having trouble connecting right now. Please contact the school directly at 031 171 0207 or buhlebemfundosec@gmail.com.';
  }
}
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'bot',
      createdAt: Date.now(),
      text: "👋 Hello! Let me help you! Whether it's admissions, boarding, fees, results, activities or anything else about Buhlebemfundo — just ask and I'll be happy to assist.",
    },
  ]);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: uid(),
          role: 'bot',
          text: 'Something went wrong. Please contact the school at 031 171 0207.',
          createdAt: Date.now(),
        },
      ]);
    } finally {
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-[#1E40AF] text-white shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-bold text-sm leading-tight truncate">Buhlebemfundo Assistant</div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-gray-50 scroll-smooth">
            {messages.map((m) => (
              ...
              >
                {m.role === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-[#1E40AF] flex items-center justify-center shrink-0 mb-0.5">
            ...
            {isTyping && (
              <div className="flex items-end gap-1.5">
                <div className="w-6 h-6 rounded-full bg-[#1E40AF] flex items-center justify-center shrink-0">
                  <Sparkles size={11} className="text-white" />
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                  {QUICK_QUESTIONS.map((q) => (
                    <button
                      key={q}
                      onClick={() => send(q)}
                      className="text-[11px] px-2.5 py-1 rounded-full bg-white border border-[#1E40AF]/25 text-[#1E40AF] hover:bg-[#1e3a8a] hover:text-white transition-colors font-medium shadow-sm"
                    >
                      {q}
                    </button>
                  ))}
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#1E40AF]/20 focus:border-[#1E40AF]/40 transition-all bg-gray-50 placeholder:text-gray-400"
              ...
              <button
                ...
                className="bg-[#1E40AF] hover:bg-[#1e3a8a]/90 text-white px-3 py-2 rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center shrink-0"
