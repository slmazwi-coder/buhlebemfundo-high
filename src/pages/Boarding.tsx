...
const inp = 'border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#1E40AF]/40 focus:border-[#1E40AF] transition w-full bg-white';
...
const SectionHeading = ({ title }: { title: string }) => (
  <div className="flex items-center gap-2 pb-2 border-b-2 border-[#1E40AF]/20 mb-5">
    <FileText size={15} className="text-[#1E40AF] shrink-0" />
...
const StepBadge = ({ num, label, active, done }: {
  ...
  <div className="flex items-center gap-2">
    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black border-2 transition-all
      ...
      : active ? 'bg-white border-white text-[#1E40AF]'
...
const FileUploadRow = ({ label, required, fileKey, files, onChange }: {
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl border-2 transition
      ${file ? 'border-[#1E40AF] bg-[#f9fafb]'
      : required ? 'border-dashed border-red-300 bg-red-50/30'
      : 'border-dashed border-gray-300 bg-gray-50'}`}>
      <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center
        ${file ? 'bg-[#1E40AF] text-white' : 'bg-gray-200 text-gray-400'}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={`text-xs mt-0.5 truncate ${file ? 'text-[#1E40AF]' : 'text-gray-400'}`}>
          {file ? file.name : 'No file chosen'}
        </div>
      </div>
      <label className="shrink-0 text-xs font-bold text-[#1E40AF] cursor-pointer hover:underline">
        {file ? 'Change' : 'Upload'}
  ...
  if (submitted) {
    return (
      <div className="py-20 flex items-center justify-center min-h-[60vh]">
        ...
        >
          <div className="w-20 h-20 bg-[#FFF3CC] text-[#1E40AF] rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle size={48} />
          </div>
  ...
  return (
    <div className="py-12 sm:py-16 bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Info banner */}
        <div className="mb-6 bg-[#f9fafb] border border-[#1E40AF] rounded-2xl p-4 flex gap-3">
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
          ...
          {/* Header */}
          <div className="bg-[#1E40AF] px-8 py-7 text-white">
              ...
              >
                    ...
                    <section>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <Field label="Name & Address of School Admitted To" className="sm:col-span-2">
                          <input className={inp} value={learner.schoolNameAndAddress} onChange={e => patchL('schoolNameAndAddress', e.target.value)} placeholder="Buhlebemfundo, Igoga Location, Matatiele" />
                        </Field>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr>
                              ...
                              <th className="text-left px-3 py-2 bg-[#f9fafb] text-xs font-bold text-[#1E40AF] uppercase">Father / Guardian / Responsible Person</th>
                              <th className="text-left px-3 py-2 bg-[#f9fafb] text-xs font-bold text-[#1E40AF] uppercase">Mother / Relative</th>
                            </tr>
                          </thead>
                          <tbody>
                            {([
                    ...
                    <section>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {htl02RequiredUploads.map(f => (
                          <div key={f.key}><FileUploadRow label={f.label} required={f.required} fileKey={f.key} files={files} onChange={handleFileChange} /></div>
                        ))}
                      </div>
                    </section>
                    ...
                    <section className="bg-gray-50 border border-gray-200 rounded-2xl p-5">
                      ...
                      <label className="flex items-start gap-3 cursor-pointer group">
                        <input type="checkbox" checked={undertaking} onChange={e => setUndertaking(e.target.checked)}
                          className="mt-0.5 w-4 h-4 accent-[#1E40AF] cursor-pointer" />
                  ...
                  <>
                    ...
                    <section>
                      ...
                      <button type="button" onClick={() => setChildren(p => [...p, { surname:'',firstName:'',dob:'',grade:'',school:'' }])}
                        className="mt-2 text-xs text-[#1E40AF] font-semibold hover:underline">+ Add another child</button>
                    </section>
                ...
                {step === 3 && (
                  <>
                    ...
                    <section className="bg-[#f9fafb] border-2 border-[#1E40AF] rounded-2xl p-6">
                      <div className="flex gap-4 items-start">
                        <div className="w-12 h-12 bg-[#FFF3CC] text-[#1E40AF] rounded-xl flex items-center justify-center shrink-0">
                          <Download size={22} />
                        </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  {step < 3 ? (
                    <button type="button" onClick={goNext}
                      className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#1E40AF] text-white text-sm font-bold hover:bg-[#B89200]/90 transition shadow">
                      Next <ChevronRight size={16} />
                    </button>
                  ) : (
                    <button type="submit" disabled={submitting}
                      className="inline-flex items-center gap-2 px-7 py-2.5 rounded-xl bg-[#1E40AF] text-white text-sm font-bold hover:bg-[#B89200]/90 disabled:opacity-60 disabled:cursor-not-allowed transition shadow">
                      {submitting
